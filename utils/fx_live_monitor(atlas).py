...
def make_market_prices_chart(
    ...
    commodity_label: str,
    commodity_ccy: str,
    twin_availability: dict[str, bool] | None = None,
    ...
    twin_availability : {label: bool} — unavailable twins get a greyed-out
                        annotation row in the legend instead of a live price line.
    price_available   : if False, the main commodity trace is shown as a dashed
                        grey line with a "Data unavailable" legend entry.
    """
    if twin_availability is None:
        twin_availability = {lbl: True for lbl in twin_prices}

    fig = go.Figure()

    main = commodity_prices.dropna()
    if price_available and not main.empty:
        fig.add_trace(go.Scatter(
            x=main.index,
            y=main.values,
            name=commodity_label,
        ...
        # Main price unavailable — draw a flat NaN trace so the legend entry appears
        # then add a centred annotation on the plot area
        _idx = main.index if not main.empty else pd.bdate_range(
            end=datetime.today(), periods=252
        )
        fig.add_trace(go.Scatter(
            x=_idx,
            y=[np.nan] * len(_idx),
            name=f"{commodity_label}  ⚠ data unavailable",
            mode="lines",
        ...
        fig.add_annotation(
            ...
            x=0.5, y=0.5,
            text=f"<b>{commodity_label}</b><br>Data unavailable at the moment",
            showarrow=False,
            font=dict(size=13, color="#8b949e", family="IBM Plex Mono"),
            align="center",
    ...
    for i, (label, series) in enumerate(twin_prices.items()):
        is_available = twin_availability.get(label, True)
        ...
        if is_available:
            ...
            fig.add_trace(go.Scatter(
                ...
                name=label,
                mode="lines",
                line=dict(color=_PAL[(available_idx + 1) % len(_PAL)], width=1.2, dash="dot"),
                yaxis="y2",
                visible=True if available_idx < 2 else "legendonly",
                hovertemplate=f"{label}: %{{y:,.1f}}<extra></extra>",
            ))
            available_idx += 1
        else:
            # Unavailable exchange — add greyed-out legend entry, no data line
            fig.add_trace(go.Scatter(
                x=[None], y=[None],
...
def render_fx_section(
    ...
    lookback_days: int = 365,
    twin_availability: dict[str, bool] | None = None,   # {label: True if live AtlasClient data}
    price_available: bool = True,                        # False if main commodity has no feed
) -> None:
    """
    Render the full FX Impact section into the active Streamlit column/container.

    twin_availability : dict mapping each twin label to True (live data via AtlasClient)
                        or False (exchange not in Barchart feed — show "Data unavailable").
                        If None, all twins are assumed to have live data.
    price_available   : True if main commodity price is live; False shows a banner.
    """
    import streamlit as st

    # Default: treat everything as available if flags not supplied
    if twin_availability is None:
        twin_availability = {lbl: True for lbl in twin_prices}
    ...
    with tab_market:
        ...
        if not price_available:
            st.warning(
                f"⚠️ **{commodity} ({exchange})** — live price data is not available "
                f"at the moment via the current feed.",
                icon=None,
            )

        unavailable_twins = [lbl for lbl, ok in twin_availability.items() if not ok]
        if unavailable_twins:
            st.info(
                "ℹ️ The following twin markets have **no live data at the moment** "
                f"(exchange not in current Barchart feed): "
                + ", ".join(f"**{t}**" for t in unavailable_twins),
                icon=None,
            )
        ...
        # Source caption reflects live vs fallback status
        _main_src = "AtlasClient (Barchart)" if price_available else "⚠ unavailable"
        _twin_src_parts = []
        for lbl, ok in twin_availability.items():
            _twin_src_parts.append(f"{lbl}: {'live' if ok else '⚠ unavailable'}")
        st.caption(
            f"Main price source: {_main_src}  ·  "
            "Unavailable exchanges: ICEUS, ICE (London), CBOT, CME, NYMEX"
        )
    ...
    with tab_norm:
        ...
        # Convert main commodity (skip if not available)
        price_map: dict[str, pd.Series] = {}
        label_main = f"{commodity} ({exchange})"

        if price_available and not commodity_prices.isna().all():
            usd_main = to_usd_per_mt(commodity_prices, currency, commodity, active_fx)
            price_map[label_main] = usd_main
        else:
            st.warning(
                f"⚠️ **{label_main}** — price data unavailable; excluded from USD normalisation.",
                icon=None,
            )

        skipped_twins: list[str] = []
        for lbl, ts in twin_prices.items():
            t_available = twin_availability.get(lbl, True)
            if not t_available or ts.isna().all():
                skipped_twins.append(lbl)
                continue
        ...
        if skipped_twins:
            st.info(
                "ℹ️ Excluded from normalised view (data unavailable): "
                + ", ".join(f"**{t}**" for t in skipped_twins),
                icon=None,
            )
        ...
        if price_map:
            ...
            st.warning("No price data available to normalise for this selection.")

        _norm_src = "AtlasClient (Barchart)" if price_available else "unavailable"
        st.caption(f"Source: Yahoo Finance (FX) · {_norm_src} (prices) · Converted at daily close")
    ...
    with tab_attr:
        ...
        st.caption("Source: Yahoo Finance (FX) · AtlasClient/Barchart (prices)")
    ...
    with tab_overlay:
        ...
        st.caption(f"Source: Yahoo Finance ({fx_pair}) · AtlasClient/Barchart (prices)")
